...
class ModelBase():
    ...
    def __init__(self, remap):
        ...
        self.arg_param_needed = set()
        self.obs_param_needed = set()
        self.set_remap(remap)

    def _set_param_needed_from_alias(self, alias, which):
        if type(alias) == str:
            self.param_needed.add(alias)
            if which == 'arg':
                self.arg_param_needed.add(alias)
            elif which == 'obs':
                self.obs_param_needed.add(alias)
        elif callable(alias):
            self.param_needed.update(inspect.getfullargspec(alias).args)
            if which == 'arg':
                self.arg_param_needed.update(inspect.getfullargspec(alias).args)
            elif which == 'obs':
                self.obs_param_needed.update(inspect.getfullargspec(alias).args)
        elif type(alias) == tuple:
            for _alias in alias:
                self._set_param_needed_from_alias(_alias, which)
    ...
    def set_remap(self, remap):
        ...
        for arg_name, alias in remap['arg'].items():
            if arg_name in self.arg_needed:
                ...
                self._set_param_needed_from_alias(alias, which='arg')
        for obs_name, alias in remap['obs'].items():
            if obs_name in self.obs_needed:
                ...
                self._set_param_needed_from_alias(alias, which='obs')

    def _evaluate_item(self, param, item):
        value = item
        if type(value) == str:
            value = param[value]
        elif type(value) == tuple:
            value = tuple([self._evaluate_item(param, x) for x in value])
        elif callable(value):
            param_param_names = inspect.getfullargspec(value).args
            value = value(**{x: param[x] for x in param_param_names})
        return value

    def _get_values_from_remap(self, param, names_needed):
        result = {}
        for name in names_needed:
            result[name] = self._evaluate_item(param, self.remap[name])
        return result
    ...
    def get_arg_from_param(self, param):
        return self._get_values_from_remap(param, self.arg_needed)

    def get_obs_from_param(self, param):
        return self._get_values_from_remap(param, self.obs_needed)

    def likelihood_from_param(self, param):
        return self.likelihood(self.get_arg_from_param(param),
                               self.get_obs_from_param(param))

    def loglikelihood_from_param(self, param):
        return self.loglikelihood(self.get_arg_from_param(param),
                                  self.get_obs_from_param(param))

    def simulate_from_param(self, param, **kwargs):
        return self.simulate(self.get_arg_from_param(param), **kwargs)
...
class Gaussian(ModelBase):
    ...
    def loglikelihood(self, arg, obs):
        return -np.log(arg['std']) - 0.5 * (obs['x'] - arg['mu'])**2 / arg['std']**2
